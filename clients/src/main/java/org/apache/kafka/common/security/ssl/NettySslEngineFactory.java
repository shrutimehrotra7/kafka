...
public class NettySslEngineFactory implements SslEngineFactory {
    ...
    protected NettySslEngineFactory.SecurityStore createKeystore(String type, String path, Password password, Password keyPassword, Password privateKey, Password certificateChain) {
        if (privateKey != null) {
            if (!PEM_TYPE.equals(type))
                ...
                return new NettySslEngineFactory.PemStore(certificateChain, privateKey, keyPassword);
        } else if (certificateChain != null) {
            ...
            if (password != null)
                ...
                return new NettySslEngineFactory.FileBasedPemStore(path, keyPassword, true);
            ...
            return new NettySslEngineFactory.FileBasedStore(type, path, password, keyPassword, true);
        } else
            return null; // path == null, clients may use this path with brokers that don't require client auth
    }

    private static NettySslEngineFactory.SecurityStore createTruststore(String type, String path, Password password, Password trustStoreCerts) {
        if (trustStoreCerts != null) {
            if (!PEM_TYPE.equals(type))
                ...
                return new NettySslEngineFactory.PemStore(trustStoreCerts);
        } else if (PEM_TYPE.equals(type) && path != null) {
            if (password != null)
                throw new InvalidConfigurationException("SSL trust store password cannot be specified for PEM format.");
            else
                return new NettySslEngineFactory.FileBasedPemStore(path, null, false);
        } else if (path == null && password != null) {
            throw new InvalidConfigurationException("SSL trust store is not specified, but trust store password is specified.");
        } else if (path != null) {
            return new NettySslEngineFactory.FileBasedStore(type, path, password, null, false);
        } else
            return null;
    }
    ...
    static class FileBasedStore implements NettySslEngineFactory.SecurityStore {
    ...
    static class FileBasedPemStore extends NettySslEngineFactory.FileBasedStore {
        ...
        protected KeyStore load(boolean isKeyStore) {
            try {
                ...
                NettySslEngineFactory.PemStore pemStore = isKeyStore ? new NettySslEngineFactory.PemStore(storeContents, storeContents, keyPassword) :
                    new NettySslEngineFactory.PemStore(storeContents);
    ...
    static class PemStore implements NettySslEngineFactory.SecurityStore {
        private static final NettySslEngineFactory.PemParser CERTIFICATE_PARSER = new NettySslEngineFactory.PemParser("CERTIFICATE");
        private static final NettySslEngineFactory.PemParser PRIVATE_KEY_PARSER = new NettySslEngineFactory.PemParser("PRIVATE KEY");
